    ...
    for(int obsIdx = 0; obsIdx < obstaclesCount_; ++obsIdx)
        {
            ...
            for(int d = 0; d < W_DIM; ++d)
                {
                    obs[d]         = obstacles_[obsIdx * 2 * W_DIM + d];
                    obs[W_DIM + d] = obstacles_[obsIdx * 2 * W_DIM + W_DIM + d];
                }
            if(x >= obs[0] - safetyMargin_ && x <= obs[3] + safetyMargin_ && y >= obs[1] - safetyMargin_ && y <= obs[4] + safetyMargin_ &&
