                                   ...
                                   const std::string& csvPath)
{
    ...
    std::vector<std::vector<std::vector<double>>> trajectories;
    {
        ...
        while(std::getline(infile, line))
            {
                ...
                // Separate trajectories by zero rows
                if(std::all_of(row.begin(), row.end(), [](double v) {
                       return v == 0.0;
                   }))
                    {
                        if(!states.empty())
        ...
        // Add the last trajectory if any
        if(!states.empty())
            {
                trajectories.push_back(states);
            }
    ...
    if(trajectories.empty())
        {
            std::cout << "No valid trajectories found in the CSV file: " << csvPath << std::endl;
            return;
        }

    // 4) Compute path costs for each trajectory
    for(size_t trajIdx = 0; trajIdx < trajectories.size(); ++trajIdx)
        {
            const auto& trajectory = trajectories[trajIdx];
            ompl::base::Cost trajectoryCost(0.0);
            ...
            for(size_t i = 1; i < trajectory.size(); ++i)
                {
                    ...
                    // Fill state variables based on MODEL
                    if(MODEL == 1)
                        {
                            ...
                            pos1->values[0] = trajectory[i - 1][0];
                            pos1->values[1] = trajectory[i - 1][1];
                            pos1->values[2] = trajectory[i - 1][2];
                            vel1->values[0] = trajectory[i - 1][3];
                            vel1->values[1] = trajectory[i - 1][4];
                            vel1->values[2] = trajectory[i - 1][5];

                            pos2->values[0] = trajectory[i][0];
                            pos2->values[1] = trajectory[i][1];
                            pos2->values[2] = trajectory[i][2];
                            vel2->values[0] = trajectory[i][3];
                            vel2->values[1] = trajectory[i][4];
                            vel2->values[2] = trajectory[i][5];
                        }
                    else if(MODEL == 2)
                        {
                            auto* pos1    = cstate1->as<ob::RealVectorStateSpace::StateType>(0);
                            ...
                            pos1->values[1]    = trajectory[i - 1][1];
                            pos1->values[2]    = trajectory[i - 1][2];
                            orient1->values[0] = trajectory[i - 1][3];
                            orient1->values[1] = trajectory[i - 1][4];
                            vel1->values[0]    = trajectory[i - 1][5];

                            pos2->values[0]    = trajectory[i][0];
                            pos2->values[1]    = trajectory[i][1];
                            pos2->values[2]    = trajectory[i][2];
                            orient2->values[0] = trajectory[i][3];
                            orient2->values[1] = trajectory[i][4];
                            vel2->values[0]    = trajectory[i][5];
                        }
                    else if(MODEL == 3)
                        {
                            auto* pos1    = cstate1->as<ob::RealVectorStateSpace::StateType>(0);
                            ...
                            pos1->values[1]    = trajectory[i - 1][1];
                            pos1->values[2]    = trajectory[i - 1][2];
                            ori1->values[0]    = trajectory[i - 1][3];
                            ori1->values[1]    = trajectory[i - 1][4];
                            ori1->values[2]    = trajectory[i - 1][5];
                            vel1->values[0]    = trajectory[i - 1][6];
                            vel1->values[1]    = trajectory[i - 1][7];
                            vel1->values[2]    = trajectory[i - 1][8];
                            angVel1->values[0] = trajectory[i - 1][9];
                            angVel1->values[1] = trajectory[i - 1][10];
                            angVel1->values[2] = trajectory[i - 1][11];

                            pos2->values[0]    = trajectory[i][0];
                            pos2->values[1]    = trajectory[i][1];
                            pos2->values[2]    = trajectory[i][2];
                            ori2->values[0]    = trajectory[i][3];
                            ori2->values[1]    = trajectory[i][4];
                            ori2->values[2]    = trajectory[i][5];
                            vel2->values[0]    = trajectory[i][6];
                            vel2->values[1]    = trajectory[i][7];
                            vel2->values[2]    = trajectory[i][8];
                            angVel2->values[0] = trajectory[i][9];
                            angVel2->values[1] = trajectory[i][10];
                            angVel2->values[2] = trajectory[i][11];
                        }

                    // Compute motion cost
                    ompl::base::Cost motionCost = obj->motionCost(s1, s2);
                    trajectoryCost              = obj->combineCosts(trajectoryCost, motionCost);
            ...
            // Print cost for the current trajectory
            std::cout << "Cost of trajectory " << trajIdx + 1 << ": " << trajectoryCost.value() << std::endl;
        }
}
